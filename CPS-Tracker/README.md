...
ICS, we first carry out static program analysis on individual
...
cd build-arm
cmake .. \
  -DCMAKE_TOOLCHAIN_FILE=../arm-toolchain-new.cmake # get this toolchain from the ../toolchain directory in cheatsheets
  -DPLATFORM=arm
...
## Built buildroot
1. Goto Chapter 3. Getting Buildroot - https://buildroot.org/downloads/manual/manual.html
2. Download one of the `2020.05.rc3` from  here - buildroot-2020.05-rc3-done
3. Now, follow these steps,
```
make menuconfig
# choose Arm (little-endian) from the architecture
# Change others if needed to and press Esc
make
```

## Docker buildroots
Tried to find if there is any `auditd` present inside docker but there is not any image containing the `auditd`.
